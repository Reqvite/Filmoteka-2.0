...
export const popularMovieMarkupSlider = async () => {
  ...
  const markup = resp.data.results.reduce((acc, { profile_path, name, id }) => {
    ...
    return (
      acc +
      `<li class="swiper-slide" data-name="actor" data-id="${id}"><img class="main-slider__img" src=${profile_path} alt="${name}"><div class="main-slider__actor-box"><p class="main-slider__actor-name">${name}</p></div></li>`
    );
  }, '');
